...
srvr.matchPlayers = function() {
    var i, s = this,
        k, cl;
    if ((i = this.searchingForGame.size) >= 2) {
        this.newGame();
    }
/*else if ( i >= 2 ) {
    for ( k in this.searchingForGame ) {
      ...
      if ( !cl.emit ) continue;
      cl.emit('foundOneMatchButWaiting')
    }
    setTimeout(function() {
      s.newGame();  
    }, 3000);
...
srvr.getClientById = function(id) {
    var c,l,cl,cls=io.sockets.clients();
...
io.sockets.on('connection', function(client) {
    ...
    client.on('disconnect', function() {
        ...
        if ((cg = client.currentGame)) {
            client.dead = true;
            io.sockets.in(cg.id).emit('playerLeft', {
                id: client.id
            });
        }
        if (srvr.searchingForGame[client.id]) {
...
var sendLobbyList = function() {
    var clients = io.sockets.clients(),cts=[],c,l,cl;
    ...
    io.sockets.emit('lobbyList',{clients:cts});
};
...
(function(scope) {
    ...
    p.addClient = function(client) {
        ...
        io.sockets.in(this.id).emit('clientList', this.clientIds);
    }
    ...
    p.start = function() {
        ...
        this.started = true;
        this.incomeCd = 0;
        io.sockets.in(this.id).emit('gameStart', {
            units: []
        });
        sendLobbyList();
    }
    ...
    p.tick = function(dF) {
        ...
        if (dc >= l - 1) {
            for (c = 0, l = (r = this.clients).length; c < l; c++) {
                u = r[c];
                if (!u.dead) {
                    io.sockets.in(this.id).emit('playerWon', {
                        id: u.id
                    });
                }
    ...
    p.tick = function(dF) {
        ...
        if (this.coolDown <= 0) {
            ...
            if (possibleTargets.length) {
                ...
                if (this.type.shoots && homeFactor == 1.25) {
                    var bullet = this.game.deployUnit(this._owner, {
                        x: this.x,
                        y: this.y,
                        type: 'bullet',
        ...
        if (!this.hitting) {
            ...
            if (this.speed < 0 && this.x < -10) {
                ...
                cl.lives--;
                if (cl.lives == 0) {
                    io.sockets.in(this.id).emit('playerDied', {
                        id: cl.id
                    });
                    cl.dead = true;
                }
